<script lang="ts">
  ...
  import { getOrder, payOrder, receiveOrder, cancelOrder, type Order, type OrderItem } from '$lib/services/orders';
  ...
  async function handleReceive() {
      ...
      paymentProcessing = true;
      try {
          const updatedOrder = await receiveOrder(order.id);
          order = updatedOrder;
          ...
          {order.status === 'completed' ||
				...
				{#if order.status === 'pending'}
					...
					<button
						...
						disabled={paymentProcessing}
						class="ml-4 px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
					>
						Cancel
					</button>
				{/if}
				{#if order.status === 'shipped'}
					<button
						...
						disabled={paymentProcessing}
						class="ml-4 px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
					>
						{paymentProcessing ? 'Processing...' : 'Mark as Received'}
					</button>
				{/if}
			</div>
			...
			<ul role="list" class="divide-y divide-gray-200">
				{#each order.items as item}
					<li class="px-4 py-4 sm:px-6 flex justify-between items-center">
						...
						<div class="text-sm font-bold text-gray-900 flex flex-col items-end">
							<span>${item.price_at_purchase * item.quantity}</span>
							{#if order.status === 'completed'}
								<a
									href="/orders/{order.id}/review/{item.product_id}"
									class="inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors border border-indigo-100"
								>
									<svg
